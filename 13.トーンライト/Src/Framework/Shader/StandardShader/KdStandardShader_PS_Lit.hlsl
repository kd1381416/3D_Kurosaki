...
{	
	...
	{
		//この中に入ってくるということはConeLightがtrueになっている
		//光源に向いたベクトルを求める
		//Math::Vecter3 toDir = pos  targetPos;
		//↑ターゲット座標からposまでのベクトルを出している
		//				↓ConeLightの発射位置
		...
		if(len<g_ConeLight.range)
		{
			...
			{
				//この中に入ってくるということはConeLightの角度内
				return float4(outColor + float3(0.3, 0, 0), 1);
			}
		}
	}
